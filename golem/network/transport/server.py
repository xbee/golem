...
from golem.network.transport.network import PortListeningInfo, PortListenInfo, SocketAddress, ConnectInfo
...
class Server(_Server):
    """ Basic server that can start listening on given port """

    def __init__(self, config_desc, network):
        """
        Create new server
        :param ClientConfigDescriptor config_desc: config descriptor for listening port
        :param Network network: network that server will use
        """
        _Server.__init__(self, config_desc, network)
    ...
    def change_config(self, config_desc):
        ...
        """
        _Server.change_config(self, config_desc)
        if self.config_desc.start_port <= self.cur_port <= self.config_desc.end_port:
            return

        if self.cur_port != 0:
            listening_info = PortListeningInfo(self.cur_port, self._stopped_callback, self._stopped_errback)
    ...
    def start_accepting(self, listening_established=None, listening_failure=None):
        ...
        listen_info = PortListenInfo(self.config_desc.start_port, self.config_desc.end_port,
                                     established, failure)
...
class PendingConnectionsServer(Server):
    """ Server that keeps a list of pending connections and tries different methods
    ...
    def _sync_pending(self):
        ...
        while len(self.pending_listenings) > 0:
            ...
            listen_info = PortListenInfo(pl.port, established_callback=pl.established, failure_callback=pl.failure)
        ...
        for conn in conns:
            if len(conn.socket_addresses) == 0:
                ...
                connect_info = ConnectInfo(conn.socket_addresses, conn.established, conn.failure)
        ...
        if cnt_time - self.last_check_listening_time > self.listening_refresh_time:
            ...
            for ol_id, listening in self.open_listenings.iteritems():
                if cnt_time - listening.time > self.listen_port_ttl:
                    self.network.stop_listening(PortListeningInfo(listening.port))
